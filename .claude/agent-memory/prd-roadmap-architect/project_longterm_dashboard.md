...
description: /dashboard/portfolio/longterm 장기투자 계좌 대시보드 P9-13~P9-26 구현 완료. PRD v4.8 / ROADMAP v5.0 반영
type: project
---

Phase 9에 장기투자 계좌 대시보드(`/dashboard/portfolio/longterm`) + 성과 분석 전용 페이지(`/dashboard/portfolio/performance`) + 보유 종목별 성과 분석(P9-26)이 추가됨. PRD v4.8 / ROADMAP v5.0에 반영.
...
- D1-10: 보유 종목별 성과 분석 (TWR/Alpha/연환산Alpha/MDR/Hit Rate/MDD/Up·Down Capture 7대 지표, KRW/USD 분리, Alpha 바 차트)
...
**P9-26 보유 종목별 성과 분석:**
- `lib/portfolio/holdings-performance.ts` — TWR/Alpha/연환산α/MDR/Hit Rate/MDD/Up·Down Capture 계산 모듈
- `app/api/portfolio/longterm/holdings-performance/route.ts` — GET API (5분 TTL 캐시, KR+US 병렬 조회, 벤치마크 ^KS11/^GSPC)
- `components/portfolio/longterm/HoldingsPerformanceTable.tsx` — Alpha 내림차순 정렬, 컬럼 상세 설명, MDR 연환산 수익률
- `components/portfolio/longterm/HoldingsAlphaBarChart.tsx` — Alpha vs 벤치마크 수평 바 차트 (에메랄드/레드, 내림차순)
- Performance Analysis 탭에 HoldingsPerformanceTable + HoldingsAlphaBarChart 배치
- "실현손익 성과 분석" 섹션(PerformanceMetrics/EquityCurveChart/BenchmarkChart/MonthlyReturnsHeatmap) 제거
- 탭명 변경: "Portfolio Analysis" → "Performance Analysis"
- MDR(Modified Dietz Return) 도입 — XIRR(Newton-Raphson) 대체, 반복 수렴 불필요, 수치 안정성 향상

**ROADMAP 태스크 ID: P9-13~P9-26**

**Why:** 장기투자 계좌의 주식+ETF+펀드(8654 계좌)를 엑셀에서 임포트하고 종목별 실현손익을 추적하기 위한 전용 대시보드 필요. 현재가 실시간 조회로 평가손익 계산 자동화. JSON 백업으로 데이터 안전성 확보. 성과 분석으로 KR/US 계좌별 수익률을 벤치마크 대비 추적. 보유 종목별 7대 지표로 개별 종목 성과를 벤치마크 알파 기준으로 평가.
**How to apply:** 포트폴리오 관련 기능 언급 시 추세추종(P9-05~P9-12)과 장기투자(P9-13~P9-26) 두 계좌 체계를 구분. D1-xx는 PRD 기능 ID, P9-xx는 ROADMAP 태스크 ID. avgCost 계산은 수수료 제외 기준. 성과 분석은 Modified Dietz + TWR 방식. 종목별 성과 수익률은 MDR(Modified Dietz Return) 사용.
