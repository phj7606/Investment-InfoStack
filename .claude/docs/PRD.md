...
> **추가**: v2.7 (2026.04.13) — Yahoo Finance + FRED API 기반 미국 시장 환경 지표 5개 동기화 차트

| ID | 기능 | 설명 | 우선순위 |
|----|-----|-----|---------|
| M7-01 | 시장 데이터 수집 통합 | `lib/fetchers/fred.ts` — `fetchFredSeries()`. BAMLH0A0HYM2(HY Spread) 수집. SDEX(꼬리 리스크)는 Yahoo Finance `^SDEX` 심볼로 직접 수집 (`fetchYahooHistory("^SDEX")`) | P1 |
| M7-02 | 5개 동기화 차트 | `MarketSyncCharts.tsx` — S&P500+NASDAQ / VIX+SDEX / VIX+VVIX / VVIX/VIX Ratio / HY Spread. Recharts ComposedChart × 5. 2개 이상 라인 차트(Chart 1·2·3)에서 레전드 항목 클릭으로 개별 라인 on/off 토글 (비활성 시 취소선+회색+opacity 처리) | P1 |
...
Next.js API Routes (app/api/market/us-analysis/)
  ↓ 병렬 수집
  ├── Yahoo Finance: ^GSPC, ^IXIC, ^VIX, ^VVIX, ^SDEX
  └── FRED API: BAMLH0A0HYM2
  ↓
