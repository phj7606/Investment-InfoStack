...
// 포지션 편집 다이얼로그 — Education·Short-term 공용
// apiBase prop으로 엔드포인트 분기:
//   "/api/portfolio/education/positions" or "/api/portfolio/shortterm/positions"
...
interface EditPositionDialogProps {
  ...
  apiBase: string;           // "/api/portfolio/education/positions" 등
...
function posToForm(p: EducationPosition): FormState {
  return {
...
export function EditPositionDialog({
  open, position, apiBase, onOpenChange, onSaved,
}: EditPositionDialogProps) {
  const [form, setForm] = useState<FormState>(() => posToForm(position));
  const [saving, setSaving] = useState(false);
  const [error, setError]   = useState<string | null>(null);

  // 포지션이 바뀔 때 폼 초기화
  useEffect(() => {
    setForm(posToForm(position));
    setError(null);
  }, [position]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-sm">포지션 편집</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              ...
              <Input className="h-8 text-xs" value={form.avgPrice}
              ...
              {(commNum > 0 || taxNum > 0) && (
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="tabular-nums text-orange-500">
                    +{(commNum + taxNum).toLocaleString()}원
                  </span>
                </div>
              )}
            </div>
