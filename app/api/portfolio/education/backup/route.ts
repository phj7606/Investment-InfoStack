/**
 * GET  /api/portfolio/education/backup  — JSON 백업 파일 다운로드
 * POST /api/portfolio/education/backup  — JSON 파일로 데이터 복원
 *
 * GET 응답:
 *   - Content-Disposition: attachment 헤더로 브라우저 다운로드 유도
 *   - 파일명: education-backup-YYYY-MM-DD.json
 ...
 *   - 응답: { ok, restoredPositions, restoredTrades, skippedPositions, skippedTrades }
 */
  ...
  try {
    ...
    return new NextResponse(JSON.stringify(payload, null, 2), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="education-backup-${today}.json"`,
      },
    });
  } catch (err) {
    console.error("[education/backup GET]", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[education/backup POST]", err);
