/**
 * GET    /api/portfolio/shortterm/trades         — 거래내역 + 성과 요약 반환
 * POST   /api/portfolio/shortterm/trades         — 거래 직접 추가
 * DELETE /api/portfolio/shortterm/trades?id=...  — 거래 삭제
 *
 * 매도 처리(포지션 → 거래내역 이동)는 별도 라우트:
 *   POST /api/portfolio/shortterm/positions/sell
 */

import { NextRequest, NextResponse } from "next/server";
import { readAccountData, writeAccountData, calcShorttermSummary } from "@/lib/portfolio/shorttermData";
import type { EducationTrade } from "@/types/portfolio";

export async function GET() {
  const data = await readAccountData();
  const summary = calcShorttermSummary(data.trades);
