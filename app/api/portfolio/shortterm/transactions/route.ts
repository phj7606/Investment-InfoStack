/**
 * GET  /api/portfolio/shortterm/transactions  — 거래 목록 조회
 * POST /api/portfolio/shortterm/transactions  — 거래 추가
 *
 * Longterm 패턴 동일 — shorttermData 스토어 사용
 */

import { NextRequest, NextResponse } from "next/server";
import { randomUUID } from "crypto";
import { readTransactions, addTransaction } from "@/lib/portfolio/shorttermData";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[shortterm/transactions GET]", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[shortterm/transactions POST]", err);
