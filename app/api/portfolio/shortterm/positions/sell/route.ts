  ...
  try {
    ...
    if (!positionId || !sellDate || !sellPrice || !quantity) {
      return NextResponse.json(
        { error: "positionId, sellDate, sellPrice, quantity 필수" },
        { status: 400 }
      );
    }
    ...
    if (quantity > pos.quantity) {
      return NextResponse.json(
        { error: `매도 수량(${quantity})이 보유 수량(${pos.quantity})을 초과합니다.` },
        { status: 400 }
      );
    }
