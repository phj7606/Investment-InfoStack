/**
 * GET /api/portfolio/shortterm/prices
 *
 * Short-term 보유 종목의 현재가 조회 (Longterm prices 패턴 동일)
 * - KR 종목: Naver Finance
...
export async function GET() {
  try {
    ...
    const cacheKey   = `portfolio-shortterm-prices-v1-${allSymbols.join("-")}`;
    ...
    console.log(`[shortterm/prices] KR ${Object.keys(krPrices).length}/${krCodes.length}, US ${Object.keys(usPrices).length}/${usSymbols.length} 조회 완료`);

    return NextResponse.json({ ...result, source: "api" });
  } catch (err) {
    console.error("[shortterm/prices GET]", err);
